...
const HomePage = () => {
  ...
  return (
    <div className="h-screen overflow-y-scroll text-white font-sans bg-[url('/bg-img.png')] bg-center bg-cover bg-no-repeat"
    >
      ...
      {/* Hero */}
      <div className=' flex flex-1 flex-col items-center justify-center px-6 pb-20 mt-8 xl:mt-28'>
        <div className='w-full max-w-2xl flex flex-col justify-center items-center'>
          ...
          <div className=' flex items-center gap-2 p-1.5 pr-3 bg-white/10  backdrop-blur-md rounded-full border border-white/20 text-[13px] text-white/90'>
            <span className='px-3 py-1 text-[11px] bg-red-700 rounded-full font-medium tracking-wide'>PROMO</span>
            <span>Create your first project free.</span>
          </div>
         {/* Title */}
         ...
         {/* Prompt input with the glassmorphic varient */}
    ...
    <div className='masked-marquee w-full mt-4 max-w-2xl overflow-hidden py-1'>
      <div className='animate-marquee gap-3'>
      {
        homeTags.map((tag,i)=>(
          ...
          className='px-4 py-1.5 border rounded-full text-sm text-white bg-white/10 birder-white/25 hover:bg-white/20 transition cursor-pointer shrink-0 font-medium'
          >{tag}</button>
        ))
      }
      </div>
      ...
      <div className='mt-12 w-full'>
        ...
        <div className="space-y-2 max-h-[80vh] overflow-y-auto pr-1">
       {
        projects.map((p)=>(
         ...
         <ClockIcon size={10}/> {moment(p.updatedAt || p.createdAt).fromNow()}
          </span>
